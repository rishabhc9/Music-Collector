        ...
        with open(HISTORY_FILE, "r") as file:
            try:
                ...
                for song in history:
                    ...
                    if "time" not in song:
                        song["date"] = song["recognized_at"].split()[1]
...
def download_song():
    ...
    if not selected_items:
        messagebox.showwarning("Download", "Please select at least one song first!")
        return
...
def delete_song():
    """Delete the selected songs from history."""
    selected_items = history_list.selection()
    if not selected_items:
        messagebox.showwarning("Delete", "Please select at least one song first!")
        return
            ...
            try:
                ...
                for song in history:
                    if filter_param == "Title" and filter_value.lower() in song.get("title", "").lower():
                        ...
                        filtered_history.append(song)
                    elif filter_param == "Time" and filter_value in song.get("time", ""):
                        filtered_history.append(song)
            ...
            try:
                ...
                for song in history:
                    if filter_param == "Title":
                        ...
                        unique_values.add(song.get("time", ""))
...
def generate_chart():
    """Generate a horizontal bar chart based on the current state of the history list."""
    ...
    for song in song_data:
        title = song[0]  # Title is the first column
        song_counts[title] += 1
...
download_btn = tk.Button(root, text="Download Selected Songs", command=download_song, **button_style)
...
filter_param_dropdown = ttk.Combobox(filter_frame, textvariable=filter_param_var, values=["Title", "Artist", "Date", "Time", "Day of Week", "Time Category"])
...
sort_column_dropdown = ttk.Combobox(sort_frame, textvariable=sort_column_var, values=["Title", "Artist", "Date", "Time", "Day of Week", "Time Category"])
...
# Add history list
columns = ("Title", "Artist", "Date", "Time", "Day of Week", "Time Category")
history_list = ttk.Treeview(root, columns=columns, show="headings", height=20, selectmode='extended')
for col in columns:
    history_list.heading(col, text=col)
    history_list.column(col, width=190)
